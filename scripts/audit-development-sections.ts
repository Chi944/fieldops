...
export async function auditDevelopmentSections(name: string, root = process.cwd()) {
  ...
  if ((await fingerprint(root, "retain_valid_chunks_v1", report.configuration.extractionTransport ?? "legacy_v5")).sha256 !== report.configuration.sha256) throw new Error("Current source/runtime differs from the finalized report; reconstruct the recorded revision before auditing its section plan.");
