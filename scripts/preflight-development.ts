...
import { assertDevelopmentEnvironment, PAIRED_COHORT, requestReservation } from "../eval/development-control";
...
export async function preflightDevelopment(name: string, root = process.cwd()) {
  ...
  const configuration = await fingerprint(root, policy);
  ...
  for (const id of PAIRED_COHORT) {
    ...
    const extractionOptions = { request, chunkFailurePolicy: policy };
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  ...
  if (args.length !== 2 || args[0] !== "--name") { console.error("Use --name <fresh-development-experiment>."); process.exitCode = 1; }
  else preflightDevelopment(args[1]).catch(error => { console.error(error instanceof Error ? error.message : "Offline preflight failed."); process.exitCode = 1; });
}
