...
export interface EvaluationOptions { live: boolean; split: "all" | "dev" | "heldout"; allowPosthoc?: boolean; waitQuota?: boolean; maxNewRequests?: number; }
...
export function parseOptions(args: string[]): EvaluationOptions {
  ...
  return { live, split: split as EvaluationOptions["split"], allowPosthoc: args.includes("--allow-posthoc"), waitQuota: args.includes("--wait-quota"), ...(maxNewRequests !== undefined ? { maxNewRequests } : {}) };
}
...
export async function evaluate(options: EvaluationOptions) {
  ...
  const checkpoint = {
    ...
    reject: async (key: string, result: AIResult, code: string, context?: { cached: boolean }) => {
      ...
      if (!context?.cached && result.finishReason && (result.finishReason !== "stop" || typeof result.data === "string" || result.data === null)) {
  ...
  const scenarioCount = new Set(documents.map(document => document.scenarioId)).size;
  ...
  const measuredReport = { ...report, runtime, modelUsage, observedUsage, liveSummary, ...(options.live ? { liveSession: { sessionId, ...liveControl.state } } : {}) };
