...
async function configHash() {
  const files = ["src/lib/ai/index.ts", "src/lib/ai/schema.ts", "src/lib/ai/groq.ts", "src/lib/processing/index.ts", "src/lib/processing/ocr.ts", "src/lib/domain/matching.ts", "src/lib/domain/calculate.ts", "src/lib/domain/numeric.ts", "src/lib/domain/billing.ts", "src/lib/domain/decisions.ts", "src/lib/domain/validation.ts", "eval/metrics.ts", "scripts/evaluate.ts"];
  return hash(JSON.stringify({ promptVersion: PROMPT_VERSION, model: liveAIConfiguration().model, files: await Promise.all(files.map(async file => ({ file, hash: hash(await readFile(path.join(ROOT, file))) }))) }));
}
...
export async function evaluate(options: EvaluationOptions) {
  ...
  const documents = manifest.documents.filter(d => options.split === "all" || d.split === options.split), configurationHash = await configHash(), datasetHash = hash(goldBytes);
  ...
  const measuredReport = { ...report, modelUsage };
