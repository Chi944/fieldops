...
export async function fingerprint(root: string, chunkFailurePolicy: DevelopmentOptions["chunkFailurePolicy"] = "reject_document", extractionTransport: DevelopmentOptions["extractionTransport"] = "legacy_v5", modelStudy?: { model: NonNullable<DevelopmentOptions["model"]> }) {
  ...
  const files = await Promise.all([...FILES, "src/lib/ai/partitioned-transport.ts", "src/lib/ai/provider-error.ts", "src/lib/ai/fact-transport.ts", "src/lib/ai/dates.ts", ...(["focused_fields_v1", "focused_fields_v2"].includes(extractionTransport) ? ["src/lib/ai/focused-transport.ts"] : []), ...(extractionTransport === "focused_fields_v2" ? ["src/lib/ai/focused-contract.ts"] : []), ...(modelStudy ? ["eval/model-study-control.ts", "eval/development-outcomes.ts", "scripts/preflight-development.ts", "scripts/compare-development.ts"] : [])].map(async file => ({ file, sha256: digest(await readFile(path.join(root, file))) })));
