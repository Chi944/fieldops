...
export async function readDevelopmentManifest(root: string): Promise<DevelopmentManifest> {
  ...
  const manifest = JSON.parse(await readFile(path.join(root, "eval/development/gold.json"), "utf8")) as DevelopmentManifest;
...
export async function loadDevelopmentEnvironment(root: string, file: ".env.ai.local" | undefined): Promise<void> {
...
export async function evaluateDevelopment(options: DevelopmentOptions, root = process.cwd()) {
  ...
  if (checked.phase === "after") {
    ...
    const before = JSON.parse(bytes.toString("utf8")) as { name: string; mode: string; phase: string; pair: typeof pair; configurationStableDuringRun: boolean; configuration: { sha256: string; model: string }; measuredAt: string };
  ...
  try {
    ...
    const plan = { pair, configurationHash: identity.sha256, ...(baselineReference ? { baselineReference } : {}) };
    ...
    const persist = (event: DevelopmentEvent) => recordEvent(journalPath, { ...event, documentId: activeDocument });
    ...
    for (let index = 0; index < fixtures.length; index++) {
      ...
      if (controller.halt) { measurements.push({ id: fixture.id, status: "not_attempted", errorCode: controller.halt }); continue; }
