...
import { access, mkdir, open, readFile, realpath, unlink, writeFile } from "node:fs/promises";
...
import { groqModelProfile } from "../src/lib/ai/groq";
import { acquireModelStudyBudget, assertComparableModelConfigurations, MODEL_STUDY_METRIC_VERSION } from "../eval/model-study-control";
...
export async function fingerprint(root: string, chunkFailurePolicy: DevelopmentOptions["chunkFailurePolicy"] = "reject_document", extractionTransport: DevelopmentOptions["extractionTransport"] = "legacy_v5", modelStudy?: { model: NonNullable<DevelopmentOptions["model"]> }) {
  ...
  const files = await Promise.all([...FILES, "src/lib/ai/partitioned-transport.ts", "src/lib/ai/provider-error.ts", "src/lib/ai/fact-transport.ts", ...(extractionTransport === "focused_fields_v1" ? ["src/lib/ai/focused-transport.ts"] : []), ...(modelStudy ? ["eval/model-study-control.ts", "scripts/preflight-development.ts", "scripts/compare-development.ts"] : [])].map(async file => ({ file, sha256: digest(await readFile(path.join(root, file))) })));
  const model = modelStudy?.model ?? liveAIConfiguration().model;
  const identity = { promptVersion: PROMPT_VERSION, model, chunkFailurePolicy, extractionTransport, ...(modelStudy ? { comparisonKind: "model" as const, modelProfile: groqModelProfile(model), allowPreviewModel: true, maxTransportAttempts: 1 as const } : {}), runtime: { node: process.version, platform: process.platform, architecture: process.arch, ocrSha256 }, files };
...
export async function evaluateDevelopment(options: DevelopmentOptions, root = process.cwd()) {
  ...
  const pair = { mode, split: "dev", selection, limits, metricVersion: modelStudy ? MODEL_STUDY_METRIC_VERSION : "fieldops-development-1", synthetic: true, ...(modelStudy ? { comparisonKind: "model" as const, models: ["openai/gpt-oss-120b", "qwen/qwen3.8-27b"], maxTransportAttempts: 1 } : {}) };
  ...
  try {
    ...
    const persist = async (event: DevelopmentEvent) => recordEvent(journalPath, { ...(sharedBudget ? await sharedBudget.annotate(event) : event), documentId: activeDocument });
    const record = async (event: DevelopmentEvent) => { await persist(event); events.push(event); };
    const checkpoint: AICheckpoint = {
      get: key => optionalJson<AIResult>(path.join(privateDirectory, "requests", `${key}.json`)),
      ...
      reject: async (key, result, code, context) => {
        ...
        if (!context?.cached && result.rejectedAt === "transport") await record(responseEvent(result));
        await record({ kind: "rejection", at: new Date().toISOString(), code, cached: context?.cached ?? false });
      },
    };
    const studyAIOptions = modelStudy ? { allowPreviewModel: true, maxTransportAttempts: 1 as const } : {};
    const controller = developmentRequestController({ limits, events, persist, ...(modelStudy ? { maxTransportAttempts: 1 as const, retryQuota: false, lastSharedActivity: sharedBudget!.lastActivity } : {}), request: (request, context) => requestAI(request, { ...context, checkpoint, ...studyAIOptions }) });
    ...
    for (let index = 0; index < fixtures.length; index++) {
      ...
      try {
        ...
        const extractionOptions = { checkpoint, request: controller.request, chunkFailurePolicy: checked.chunkFailurePolicy, extractionTransport: checked.extractionTransport, ...studyAIOptions };
