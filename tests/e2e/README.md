...
The subsequent UI refinement was verified against the local **production build** with AI disabled: **7/7 browser tests passed in 27.7 seconds**. The added `refinements.spec.ts` checks Ctrl/Cmd+K search (and no unmodified character shortcut), clear/filter recovery, accessible next-step labels, deep links to the quotation with issues, actual discrepancy evidence, missing-source behavior, correction evidence, unmatched filters and keeping keyboard focus visible on mobile. All existing upload, export, keyboard and axe checks passed again. The final public deployment passed **5/5 browser tests in 25.9 seconds**, excluding the two private local-upload tests. Current UI screenshots in `docs/images/*-v2.png` come from that public run using only fictional sample data.

The personal-use milestone passes **8/8 browser cases in 24.8 seconds** against the local production build. `personal.spec.ts` adds two real synthetic text quotations through source-ready manual review, source-linked corrections/items, explicit completion, approved matching, incomplete-cost safeguards, Excel round-trip, print invocation/Chromium PDF and reload persistence. Personal comparisons remain separate from samples and do not enter demo localStorage. Three upload cases are local-only; five cases also target the public demo. Test-owned private comparisons are deleted in cleanup. No actual personal documents or model calls are used.
