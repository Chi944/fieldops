...
describe("foreground deletion with injected private transports", () => {
  it("commits deletion before a bounded sweep, awaits its cancellation and retains the cleanup tombstone", async () => {
    ...
    transport.sql.mockImplementation(async (query: string) => {
      ...
      if (query === "select public.fieldops_expire_uploads()") { events.push("intent-expiry"); return [{ result: 0 }]; }
    ...
    try {
      await vi.waitFor(() => expect(events).toEqual(["committed", "intent-expiry", "outbox-read", "storage-started"]));
    ...
    expect(transport.sql).toHaveBeenCalledTimes(3);
    expect(transport.sql.mock.calls[0][1]).toEqual([owner, resource]);
    expect(transport.sql.mock.calls[1][2]).toEqual({ signal: deadline.signal });
  });

  it("acknowledges committed source deletion when object cleanup fails, preserving its outbox entry", async () => {
    transport.sql.mockResolvedValueOnce([{ result: null }]).mockResolvedValueOnce([{ result: 0 }]).mockResolvedValueOnce([tombstone]);
  ...
  it("does not turn a post-commit outbox-read failure into a failed deletion response", async () => {
    transport.sql.mockResolvedValueOnce([{ result: null }]).mockResolvedValueOnce([{ result: 0 }]).mockRejectedValueOnce(new Error("Synthetic database maintenance outage"));
  ...
  it("shares the cleanup deadline with intent expiry and starts no later storage work after it expires", async () => {
    const deadline = new AbortController();
    transport.sql.mockImplementation(async (query: string, _params, options: { signal: AbortSignal }) => {
      expect(query).toBe("select public.fieldops_expire_uploads()");
      expect(options.signal).toBe(deadline.signal);
      deadline.abort();
      throw new Error("Synthetic expired database deadline");
    });
    await expect(new CloudRepository().cleanup({ signal: deadline.signal })).resolves.toBeUndefined();
