...
/** Compact transport is expanded, then subjected to the unchanged domain/evidence validators. */
export function expandExtraction(data: unknown, targetIds: string[], contextIds: string[] = []): ExtractedChunk {
  ...
  for (const disposition of wire.excluded) {
    ...
    for (const sourceId of disposition.sourceIds) {
      if (!targets.has(sourceId) || used.has(sourceId) || excluded.has(sourceId)) return invalid("Source coverage contains an unknown, repeated or already-used exclusion.");
  ...
  for (const sourceId of targetIds) {
    const coverage = used.has(sourceId) ? { sourceId, disposition: "used" as const, reason: "Referenced by extracted evidence" } : excluded.get(sourceId);
  ...
  for (const sourceId of contextIds) chunk.coverage.push({ sourceId, disposition: used.has(sourceId) ? "used" : "header", reason: "Read-only source context" });
