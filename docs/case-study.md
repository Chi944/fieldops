...
The first replay was preserved before a second concrete failure was fixed: an explicit tier range's upper bound was read as a negative number by generic decimal evidence checking. A narrow guard now accepts only a complete same-item tier whose endpoints, optional price and shared source agree with the unchanged raw clause. Generic money validation remains strict. A separately named replay of the same responses then retained three partial quotations, 47/129 correct critical fields and 7/18 identifier-aligned expected items. The original live result and first replay remain unchanged; there were no new model calls.

The latest result still has 0/3 complete quotations, 0/18 fully correct source-linked annotated items and 0/3 readiness passes. All 117 field references resolve and all original source arrays survive, but only 41/129 critical fields are correct with matching source locations. Those checks do not prove semantic entailment. All 10 retained comparison rows remain blocked from cost recommendations. The improvement fixes loss of usable data while keeping the unresolved reliability problem visible; production AI remains off.
