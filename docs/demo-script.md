...
| 2:45 | Show the evaluation report. | “The offline benchmark contains 24 authored documents and 144 items. The simple baseline achieved 102 correct equivalent pairs from 102 proposed, and found 102 of 116 true pairs. The free model completed two tiny examples, but larger development probes still failed. Hosted AI stays off, and held-out accuracy has not been established.” |
...
For real hosted uploads, demonstrate the verified parser/manual-review behavior; automatic extraction remains disabled. Never call saved sample extraction a live model result. Private cloud authentication, storage, workers and model integration should be demonstrated only after those services have actually been configured and verified. The public fixture demonstration is deployed at https://fieldops-eight-blue.vercel.app.
