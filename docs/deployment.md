...
New database, Auth, storage and production Trigger settings are saved as sensitive Vercel production variables. Only the restricted database and storage settings go to Trigger; migration-owner and OAuth secrets do not. Hosted processing is initially parser-only while live model reliability is evaluated. Provider configuration is not hosted acceptance: the refreshed web deployment, GitHub sign-in and production task workflow still need the separately recorded smoke checks.
