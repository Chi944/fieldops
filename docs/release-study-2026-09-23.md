...
The tier-range implementation and measured replay are preserved at **`d5534a2`**. All 30 fingerprinted source files and the replay helper hash matched that staged snapshot before commit. It passed 485 unit/integration tests, ten local browser workflows, the production build and independent Linux CI, then merged in PR #2 as `51c7a02`. The Git-linked production deployment and six applicable public browser checks also passed. [Release verification](release-verification.md) records the identities, exact scope and remaining gates separately from model measurements.
